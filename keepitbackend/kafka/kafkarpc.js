...
KafkaRPC.prototype.makeRequest = function(topic_name, content, callback){
    ...
    self.setupResponseQueue(self.producer,topic_name,function(){
        ...
        var payloads = [
            { topic: topic_name+"_req", messages: JSON.stringify({
                correlationId:correlationId,
                replyTo:topic_name+"_res",
                data:content}),
                partition:0}
        ];
        //console.log('in response1');
        //console.log(self.producer.ready);
        //console.log("rpc payload: ",payloads);
...
KafkaRPC.prototype.setupResponseQueue = function(producer,topic_name, next){
    ...
    var consumer = self.connection.getConsumer(topic_name+"_res");
    consumer.on('message', function (message) {
        //console.log('consumer msg received');
        var data = JSON.parse(message.value);
        //console.log("consumer data:", data);
        //console.log("consumer correlationId:", data.correlationId);
        ...
        if(correlationId in self.requests){
            ...
            //callback, no err
            //console.log("rpc consumer data.error:",data.error);
